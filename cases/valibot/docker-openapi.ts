...
export const Mount = v.object({
  ...
  BindOptions: v.optional(
    v.object({
      ...
      ReadOnlyNonRecursive: v.optional(v.boolean()),
      ReadOnlyForceRecursive: v.optional(v.boolean()),
    })
  ),
...
export const HealthConfig = v.object({
  ...
  StartInterval: v.optional(v.number()),
});
...
export const HostConfig = v.merge([
  ...
  v.object({
    ...
    ConsoleSize: v.optional(
      v.union([v.array(v.number()), v.any(/* unsupported */)])
    ),
    Annotations: v.optional(v.unknown()),
    CapAdd: v.optional(v.array(v.string())),
    CapDrop: v.optional(v.array(v.string())),
    CgroupnsMode: v.optional(
      v.union([v.literal("private"), v.literal("host")])
    ),
    ...
    Isolation: v.optional(
      v.union([v.literal("default"), v.literal("process"), v.literal("hyperv")])
    ),
...
export const EndpointSettings = v.object({
  ...
  DNSNames: v.optional(v.array(v.string())),
});
...
export const NetworkSettings = v.object({
  ...
  SecondaryIPAddresses: v.optional(
    v.union([v.array(Address), v.any(/* unsupported */)])
  ),
...
export const ImageInspect = v.object({
  ...
  Created: v.optional(v.union([v.string(), v.any(/* unsupported */)])),
  ...
  RootFS: v.optional(
    v.object({
      Type: v.string(),
      Layers: v.optional(
        v.union([v.array(v.string()), v.any(/* unsupported */)])
      ),
    })
  ),
  Metadata: v.optional(
  ...
  AccessMode: v.optional(
    v.object({
      ...
      Availability: v.optional(
        v.union([v.literal("active"), v.literal("pause"), v.literal("drain")])
      ),
    })
  ),
});
...
export const Plugin = v.object({
  ...
  Config: v.object({
    ...
    Interface: v.object({
      ...
      Socket: v.string(),
      ProtocolScheme: v.optional(
        v.union([
          v.literal(""),
          v.literal("moby.plugins.http/v1"),
          v.any(/* unsupported */),
        ])
      ),
    }),
...
export const Reachability = v.union([
  v.literal("unknown"),
      ...
      Privileges: v.optional(
        v.object({
          ...
          AppArmor: v.optional(
            v.object({
              Mode: v.optional(
                v.union([v.literal("default"), v.literal("disabled")])
              ),
            })
          ),
          NoNewPrivileges: v.optional(v.boolean()),
        })
      ),
      TTY: v.optional(v.boolean()),
      OpenStdin: v.optional(v.boolean()),
  ...
  RestartPolicy: v.optional(
    v.object({
      Condition: v.optional(
        v.union([v.literal("none"), v.literal("on-failure"), v.literal("any")])
      ),
...
export type ContainerStatus = v.Output<typeof ContainerStatus>;
export const ContainerStatus = v.object({
  ContainerID: v.optional(v.string()),
...
export const EndpointPortConfig = v.object({
  Name: v.optional(v.string()),
  Protocol: v.optional(
    v.union([v.literal("tcp"), v.literal("udp"), v.literal("sctp")])
  ),
...
export type PortStatus = v.Output<typeof PortStatus>;
export const PortStatus = v.object({
  Ports: v.optional(v.array(EndpointPortConfig)),
});

export type TaskStatus = v.Output<typeof TaskStatus>;
export const TaskStatus = v.object({
  Timestamp: v.optional(v.string()),
  State: v.optional(TaskState),
  Message: v.optional(v.string()),
  Err: v.optional(v.string()),
  ContainerStatus: v.optional(ContainerStatus),
  PortStatus: v.optional(PortStatus),
});
...
export const ServiceSpec = v.object({
  ...
  UpdateConfig: v.optional(
    v.object({
      ...
      Order: v.optional(
        v.union([v.literal("stop-first"), v.literal("start-first")])
      ),
    })
  ),
  RollbackConfig: v.optional(
    v.object({
      ...
      Delay: v.optional(v.number()),
      FailureAction: v.optional(
        v.union([v.literal("continue"), v.literal("pause")])
      ),
      Monitor: v.optional(v.number()),
      MaxFailureRatio: v.optional(v.number()),
      Order: v.optional(
...
export type ServiceCreateResponse = v.Output<typeof ServiceCreateResponse>;
export const ServiceCreateResponse = v.object({
  ID: v.optional(v.string()),
  Warnings: v.optional(
    v.union([v.array(v.string()), v.any(/* unsupported */)])
  ),
...
export const ContainerWaitResponse = v.object({
  StatusCode: v.number(),
  Error: v.optional(
    v.union([ContainerWaitExitError, v.any(/* unsupported */)])
  ),
});
...
export const SwarmInfo = v.object({
  ...
  RemoteManagers: v.optional(
    v.union([v.array(PeerNode), v.any(/* unsupported */)])
  ),
...
export const SystemInfo = v.object({
  ...
  CgroupDriver: v.optional(
    v.union([v.literal("cgroupfs"), v.literal("systemd"), v.literal("none")])
  ),
  ...
  Isolation: v.optional(
    v.union([v.literal("default"), v.literal("hyperv"), v.literal("process")])
  ),
  ...
  Warnings: v.optional(v.array(v.string())),
  CDISpecDirs: v.optional(v.array(v.string())),
});
...
export type get_ContainerAttachWebsocket = v.Output<
...
export type head_ContainerArchiveInfo = v.Output<
...
export const post_NetworkCreate = v.object({
  ...
  parameters: v.object({
    body: v.object({
      Name: v.string(),
      CheckDuplicate: v.optional(
        v.union([v.boolean(), v.any(/* unsupported */)])
      ),
...
export const post_ServiceUpdate = v.object({
  ...
  parameters: v.object({
    query: v.object({
      version: v.number(),
      registryAuthFrom: v.union([
        v.literal("spec"),
        v.literal("previous-spec"),
      ]),
      rollback: v.string(),
    }),
    path: v.object({
